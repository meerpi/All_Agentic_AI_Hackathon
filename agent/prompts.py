...
3. **Cross-Step Dynamic Referencing**: When a step depends on an artifact or ID from a previous step, reference it using `$step_<number>.<key>` (for example: `$step_1.url`, `$step_1.document_id`, `$step_2.video_title`, `$step_2.link`, `$step_3.spreadsheet_id`).
4. **Rich Tool Payloads**: When creating documents, proposals, emails, or spreadsheets, generate the complete, high-quality, professional text content in the `tool_args` (e.g. `content`, `body`, `title`, `rows`, `summary`) so the tool writes real, rich deliverables.
